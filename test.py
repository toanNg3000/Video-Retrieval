...
with gr.Blocks() as demo:
    ...
    list_test = []
    def add_selected_idx(evt: gr.SelectData):
        idx = evt.index
        img_info = evt.value
        img_path = img_info['image']['path']
        print(img_path)
        if idx not in list_test:
            list_test.append(idx)
        return list_test
    
    
    list_selected_img_idx = gr.Textbox()
    gallery.select(add_selected_idx, None, list_selected_img_idx)
